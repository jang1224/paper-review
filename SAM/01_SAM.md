...
일반적 접근은 Ls(w)를 LD(w)의 추정치로 삼아, minw Ls(w)를 SGD, Adam 등으로 최적화하는 것이다. 그러나 과매개변수화된 현대 모델에서는 이 방식이 테스트 성능에서 최적을 보장하지 않는다. Ls(w)는 비블록이며, 여러 최소점이 있고, 같은 훈련 손실이어도 일반화는 크게 달라질 수 있다.
...
여기서 p >= 0는 이웃 크기이며, p∈[1,∞]에서 p=2가 대체로 최적이였다. 위의 fig1을 보면 일반 훈련과 SAM으로 도달한 최소점의 손실 지형을 비교하는데, SAM은 날카로운 최소점으로의 수렴을 방지하는 것을 볼 수 있다.

LS^SAM(w)를 최소화하기 위해, 우리는 내부 최대화를 통해 미분하여 ∇𝑤𝐿𝑆^SAM(𝑤)의 효율적 근사를 도출한다. 먼저 𝐿𝑆(w + ϵ)을 ϵ = 0 주위에서 1차 테일러 전개하면 아래와 같은 식이 나온다.LS(w)는 상수 취급해서 무시해준다.

![taylor](./images/taylor.png)

이는 고전적인 dual norm 문제의 해로 주어진다. ϵ의 최적해는 ϵ^(w)=ρ “p-볼에서 g를 가장 잘 밀어올리는 방향” / 정규화. 아래의 식이 이런 의미를 담고 있다.

![고전적인식](./images/before_2.png)
...
2. dual norm이란?
    - 함수 분석 에서 이중 놈은 노름된 벡터 공간 에서 정의된 연속 선형 함수 의 크기를 측정하는 것. 
3. 고윳값이 낮다는건 뭐를 의미하는걸까?
    - 짧게 말하면 고윳값(특히 해시안의 고윳값) 분포가 낮다는 건, 손실 지형의 곡률이 전반적으로 작다 → 바닥이 평평하다(덜 뾰족하다) 는 뜻
4. Cutout과 AutoAugment는 뭘까?
    - Cutout: 이미지에서 임의의 사각형 영역을 잘라(0으로 가림) 학습시키는 증강. 부분 가려짐·오클루전에 강해지게 하고 과적합을 줄여줌.
    - AutoAugment: 회전·전단(shear)·색/명암 변화·좌우반전 등 여러 증강 연산의 조합(policy) 을 데이터셋별로 탐색해(RL/진화) 얻은 최적 정책을 사용하는 방법
5. MentorMix?
    - 라벨 노이즈에 강하게 학습하려고 만든 방법(멘토멘티 아이디어 + MixUp) -> 노이즈에 매우 강함
6. Bootstrap?
    - 노이즈 라벨을 모델이 스스로 정제해서 다시 학습하는 두 단계 절차.
