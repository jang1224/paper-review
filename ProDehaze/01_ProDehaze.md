...
3. **DCP 계산 방식은 어떻게 되는가?**

    ![DCP](./images/DCP.png)

    - `I_c(y)` : RGB 채널 중 하나에서 위치 y에서 채널 c의 픽셀 스케일
    - `Ω(x)` : 픽셀 x 주변의 작은 윈도우 (보통 15x15)
    
    깨끗한 이미지에서는 최소한 한 개의 컬러 채널이 아주 어두운 픽셀을 가지고 있다는 자연 이미지의 통계적 특성을 기반으로 함.

    **계산 순서**
    1. 픽셀 x를 중심으로 한 15x15 영역을 잡음.
    2. 윈도우 내에서 최소값을 찾음.
    3. 각 채널의 최솟값을 한 번 더 비교해서 최솟값을 찾음.
    4. 이 과정을 모든 픽셀에 거치면 하나의 흑백 이미지가 나옴.
